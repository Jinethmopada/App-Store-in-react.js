...
class AppStore extends Component {
  ...
  getSearchedApps = () => {
    ...
    const searchAppNames = appsList.filter(eachAppName =>
      eachAppName.appName.lowerCase().includes(searchInput.lowerCase()),
    )

    return searchAppNames
  }
      ...
      <div className="bg-container">
        ...
        <ul className="apps-container">
          {filteredApps.map(eachApp => (
            <AppItem key={eachApp.appId} appItems={eachApp} />
          ))}
        </ul>
      </div>
    )
